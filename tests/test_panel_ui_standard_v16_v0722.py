...
class PanelUiStandardV16V0722Tests(unittest.TestCase):
    ...
    def test_indicator_is_pointwise_and_lider_styled(self) -> None:
        ...
        self.assertIn(".connection-indicator.local{color:var(--success-color,#43a047);background:color-mix(in srgb,var(--success-color,#43a047) 11%,var(--card-background-color))", self.source)
        self.assertIn("border-color:color-mix(in srgb,var(--success-color,#43a047) 30%,transparent)", self.source)
    ...
    def test_two_finger_reset_blocks_hold_and_synthetic_clicks(self) -> None:
        ...
        self.assertIn("this._queuePointerTwoFingerTap(now)", gesture)
        self.assertIn('addEventListener("touchstart"', gesture)
        self.assertIn('addEventListener("touchend"', gesture)
        self.assertIn("this._registerTwoFingerTap(now)", gesture)
        self.assertIn("this._resetTransform(true)", self.source)
