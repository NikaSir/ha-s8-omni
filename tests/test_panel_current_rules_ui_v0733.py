...
class PanelCurrentRulesUiV0733Tests(unittest.TestCase):
    ...
    def test_child_modules_are_cache_busted_with_release_version(self) -> None:
        self.assertIn('import "./s8-omni-panel.js?v=1.0.10";', self.bootstrap)
        self.assertIn('import "./s8-omni-cleaning-presets.js?v=1.0.10";', self.bootstrap)
        self.assertIn('import "./s8-omni-service-settings.js?v=1.0.10";', self.bootstrap)
        self.assertIn('import "./s8-omni-preset-live-highlight.js?v=1.0.10";', self.bootstrap)
    ...
    def test_user_preset_highlight_reads_entry_and_legacy_default_storage(self) -> None:
        ...
        self.assertIn('selectedKeyForEntry("default")', self.service)
        self.assertIn('selectedKeyForEntry("default")', self.live_highlight)
        self.assertIn('PRESET_KEYS.filter((key) => presetMatches(panel, key))', self.service)
        self.assertIn('PRESET_KEYS.filter((key) => presetMatches(panel, key))', self.live_highlight)
    ...
    def test_version_metadata_cannot_drift_from_runtime(self) -> None:
        ...
        self.assertEqual("1.0.10", standard["ui_version"])
        self.assertIn('const UI_VERSION = "v1.0.10"', self.source)
        self.assertIn('VERSION = "v1.0.9-beta002"', constants)
        self.assertIn('DASHBOARD_VERSION = "v1.0.10"', constants)
        self.assertEqual("1.0.9-beta002", manifest["version"])
        self.assertEqual("v1.0.10", panel["dashboard_version"])
