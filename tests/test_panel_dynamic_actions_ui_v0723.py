...
class PanelDynamicActionsUiV0723Tests(unittest.TestCase):
    ...
    def test_runtime_and_manifest_versions_match(self) -> None:
        self.assertIn('const UI_VERSION = "v0.7.25"', self.source)
        ...
        self.assertIn('VERSION = "v1.00_b061"', constants)
        self.assertIn('DASHBOARD_VERSION = "v0.7.25"', constants)
        self.assertEqual("1.0.0b61", manifest["version"])
        self.assertEqual("v0.7.25", panel["dashboard_version"])
