...
class PanelMobileAuditUiV0729Tests(unittest.TestCase):
    ...
    def test_header_center_returns_to_parent(self) -> None:
        ...
        self.assertIn('"/dashboard-house-v11/home"', self.source)
        self.assertIn('"/dashboard-actions/home"', self.source)
        self.assertIn('"/dashboard-infrastructure/overview"', self.source)
        self.assertIn('safeReturnRoute(current.searchParams.get("return_to")) || safeReturnRoute(current.searchParams.get("from"))', self.source)
