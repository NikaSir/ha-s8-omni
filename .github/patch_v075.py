...
CLEAN = Path(".github/product_clean_v075.b64")
DOCK = Path(".github/product_dock_v075.b64")

text = JS.read_text(encoding="utf-8")
clean_b64 = CLEAN.read_text(encoding="ascii").strip()
dock_b64 = DOCK.read_text(encoding="ascii").strip()
if not clean_b64 or not dock_b64:
    raise SystemExit("approved product-art sources are empty")
...
if count != 1:
    raise SystemExit(f"product-art runtime replacement failed: {count}")
...
text = re.sub(
    r'\.tank-glow,\.charge-glow\{.*?\}\.omni-legend\{',
    '.omni-legend{',
    text,
    count=1,
    flags=re.S,
...
new_markup = '''          <img class="product-art" src="${productArtData(artMode)}" alt="S8 OMNI robot and station" />'''
if old_markup not in text:
    raise SystemExit("current product-art markup not found")
text = text.replace(old_markup, new_markup, 1)

# Keep the narrow-layout split balanced and readable.
