...
- **Return home during active cleaning:** write `power_go=false` → `pause=true`, treat `paused` as an intermediate state and wait for factual `status=standby`, then write `mode=chargego` and wait up to 30 seconds for `status=goto_charge|repositing|charging|charge_done`. The 2026-09-01 real-device trace showed direct `chargego` being acknowledged three times without movement; after Pause reached `standby`, `chargego` produced `goto_charge` after about 15 seconds and `charging` after another 25 seconds.
