...
Dashboard `v0.7.45` applies these mobile rules:
...
Volume, DND and child-lock changes stay in one local draft until Apply is confirmed, and each changed value requires entity-state readback. Cancel discards the whole draft without writing to the device. Child lock is directly beside DND and participates in the same confirmation and Apply workflow. DND wording explains that its restrictions apply during the configured hours; the period itself is configured in the official application.
...
Each child import is also cache-busted with the integration version (`1.0.0b98`). This prevents a refreshed bootstrap from loading an older child module from browser cache.
