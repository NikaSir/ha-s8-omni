...
| Pointwise two-level connection indicator | PASS | The requested indicator keeps a stable node; only channel/freshness categories, lamp, text, classes and values change. The primary status drives its LIDER tint/border and uses canonical 16/13 px typography. |
